...
export const fetchMedicines = async (query = {}) => {
  ...
  const response = await axios.get(`${getApiUrl()}/api/v1/medicine?${params}`);
  return response.data; // Assumes API response includes `data` and `meta`
};

export const deleteMedicineById = async (id) => {
  try {
    const response = await axios.delete(`${getApiUrl()}/api/v1/medicine/delete-medicine/${id}`, {
...
export const addProduct = async (product) => {
  try {
    ...
    const response = await fetch(`${getApiUrl()}/api/v1/medicine/add-medicine`, {
      method: "POST",
      headers: {
...
export const updateMedicineById = async (id, updateData) => {
  try {
    ...
    const response = await axios.put(
      `${getApiUrl()}/api/v1/medicine/update-medicine/${updateData._id}`,
      updateData,
      {
        headers: {
